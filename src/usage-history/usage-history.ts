...
export const USAGE_HISTORY_PROMPT = `Create a Pi usage report for all of my Pi sessions over the last 1, 7, 30, and 90 days.
...
export async function runUsageHistory(
	pi: ExtensionAPI,
	ctx: ExtensionCommandContext,
): Promise<void> {
	if (guardObservableCommand("/usage-history", ctx) === "stop") return;
	await ctx.waitForIdle();
	pi.sendUserMessage(USAGE_HISTORY_PROMPT);
}

export function registerUsageHistory(pi: ExtensionAPI): void {
	pi.registerCommand("usage-history", {
		description:
			"Ask the agent to summarize historical Pi/Codex CLI usage and cost for the last 1, 7, 30, and 90 days",
