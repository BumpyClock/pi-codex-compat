...
export default function usageExtension(
	...
	const commandHandler = async (args: string, ctx: ExtensionCommandContext) => {
		if (guardTuiOnlyMenu("/usage", ctx) === "stop") return;
		if (args.trim()) {
	...
	const api = {
		openMenu: commandHandler,
	};

	if (options.registerCommand !== false) {
		pi.registerCommand("usage", {
			description: "Show live OpenAI Codex subscription quota for the current runtime account",
