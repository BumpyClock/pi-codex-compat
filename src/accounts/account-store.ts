...
export class AccountStore {
	...
	private backend: AccountStorageBackend | undefined;
	private migrationNotice: string | undefined;
	private defaultBackendPending = false;

	/**
	 * When no backend is provided, the default file backend and legacy migrations are lazy.
	 * Factory-only loads must not create or migrate account files until first real use.
	 */
	constructor(backend?: AccountStorageBackend) {
		if (backend) this.backend = backend;
		else this.defaultBackendPending = true;
	}

	/** Ensure default backend/migration ran; return and clear any migration notice. */
	consumeMigrationNotice(): string | undefined {
		this.ensureBackend();
		const notice = this.migrationNotice;
		this.migrationNotice = undefined;
		return notice;
	}

	private ensureBackend(): AccountStorageBackend {
		if (this.backend) return this.backend;
		if (!this.defaultBackendPending) {
			throw new Error("AccountStore backend is not configured.");
		}
		const created = createDefaultBackend();
		this.backend = created.backend;
		this.migrationNotice = created.notice;
		this.defaultBackendPending = false;
		return this.backend;
	}

	read(): AccountsData {
		return this.ensureBackend().withLock((current) => ({ result: parseAccountsData(current) }));
	}

	async readAsync(): Promise<AccountsData> {
		return this.ensureBackend().withLockAsync(async (current) => ({
			result: parseAccountsData(current),
		}));
	}
	...
	async writeRawForTest(raw: string): Promise<void> {
		...
		await this.serialized(async () =>
			backend.withLockAsync(async () => ({ result: undefined, next: raw })),
		);
	}
	...
	return backend.withLockAsync<MigrationResult>(async (raw) => {
		...
		return {
			result: {
				...
				notice: `${LEGACY_CODEX_ACCOUNTS_FILE} was copied into ${ACCOUNTS_FILE}. The private legacy file was retained for rollback but may become stale after OAuth refresh; do not load pi-codex-accounts together with pi-codex-compat.`,
			},
		};
	});
}
...
function createDefaultBackend(): { backend: AccountStorageBackend; notice?: string } {
	const agentDir = getAgentDir();
	const canonical = join(agentDir, ACCOUNTS_FILE);
	let notice: string | undefined;
	if (!pathEntryExists(canonical)) {
		const multiPath = join(agentDir, LEGACY_MULTI_ACCOUNTS_FILE);
		if (pathEntryExists(multiPath)) {
			const result = migrateLegacyMultiAccountsFileSync(multiPath, canonical);
	...
	if (!pathEntryExists(canonical)) {
		...
		for (const legacy of legacyCandidates) {
			if (!pathEntryExists(legacy)) continue;
			const result = migrateLegacyCodexAccountsFileSync(legacy, canonical);
			if (result.notice) notice = result.notice;
			if (result.status !== "missing") break;
		}
	}
	return { backend: new FileAccountStorageBackend(canonical), notice };
}

function migrateLegacyMultiAccountsFileSync(
	legacyPath: string,
	canonicalPath: string,
): MigrationResult {
	cleanupStaleMigrationTemps(canonicalPath);
	...
	return new FileAccountStorageBackend(legacyPath, {
		...
		const contents = stringifyAccountsData(extractCodexProviderFromMultiAccounts(raw));
		try {
			installPrivateFileExclusively(canonicalPath, contents);
		} catch (error) {
			if (hasErrorCode(error, "EEXIST")) {
		...
		return {
			result: {
				status: "migrated" as const,
				notice: `${LEGACY_MULTI_ACCOUNTS_FILE} openai-codex accounts were copied into ${ACCOUNTS_FILE}. The private legacy file was retained for rollback; do not load pi-accounts together with pi-codex-compat.`,
			},
		};
	});
}

function extractCodexProviderFromMultiAccounts(raw: string | undefined): AccountsData {
	if (!raw?.trim()) return emptyAccountsData();
	let parsed: unknown;
	try {
		parsed = JSON.parse(raw) as unknown;
		...
		throw new Error(
			`Invalid legacy multi-provider accounts JSON in ${LEGACY_MULTI_ACCOUNTS_FILE}.`,
		);
	}
	if (!isRecord(parsed) || parsed.version !== 1 || !isRecord(parsed.providers)) {
		throw new Error(
			`Invalid legacy multi-provider accounts data in ${LEGACY_MULTI_ACCOUNTS_FILE}.`,
		);
	}
	const codex = parsed.providers["openai-codex"];
	if (codex === undefined) return emptyAccountsData();
	const state = normalizeProviderState(codex);
	return {
		version: 1,
		providers: defineOwn(
			Object.create(null) as Record<string, ProviderAccountsData>,
...
/**
 * @deprecated Prefer AccountStore#consumeMigrationNotice after lazy backend init.
 * Kept as a no-op export for compatibility with older call sites.
 */
