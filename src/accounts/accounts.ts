...
import { guardTuiOnlyMenu } from "../shared/mode.js";
import {
	AccountStore,
	defineOwn,
	defineOwnMap,
...
export default function accountsExtension(
	...
	// Migration is lazy on the default store; notice is collected on first session_start use.
	const providers = [
	...
	if (dependencies.registerCommand !== false) {
	...
	pi.on("session_start", async (_event, ctx) => {
		// First session sync triggers lazy backend/migration for the default store.
		const notice = dependencies.store === undefined ? store.consumeMigrationNotice() : undefined;
