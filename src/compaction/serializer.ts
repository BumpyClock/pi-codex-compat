...
/** Minimal preparation surface used by the serializer helpers. */
export type CompactionPreparation = {
	firstKeptEntryId: string;
	messagesToSummarize: import("@earendil-works/pi-agent-core").AgentMessage[];
	turnPrefixMessages: import("@earendil-works/pi-agent-core").AgentMessage[];
	isSplitTurn: boolean;
	tokensBefore: number;
...
import type { ResponsesCompatibleRequestPayload } from "./runtime.js";
...
export function collectCompactionWindowMessages(
...
export function compareCompactRequestToPayload(
	...
	if ((payload.instructions ?? "") !== request.instructions) {
		mismatches.unshift(
			"instructions: expected serialized instructions to match payload instructions",
		);
	}
...
function transformMessagesForResponses(messages: Message[]): Message[] {
	...
	for (const message of messages) {
		if (message.role === "assistant") {
			...
			const normalizedContent = message.content.flatMap(
				(block): Array<TextContent | ThinkingContent | ToolCall> => {
			...
			const normalizedAssistantMessage: AssistantMessage = {
				...message,
				content: normalizedContent as AssistantMessage["content"],
			};
...
function parseTextSignature(signature: string | undefined): ParsedTextSignature | undefined {
	...
	try {
		...
		return {
			id: record.id,
			phase:
				record.phase === "commentary" || record.phase === "final_answer" ? record.phase : undefined,
		};
	} catch {
		return undefined;
	}
...
function describeResponsesInputItem(item: unknown): string {
	...
	if (type === "message") {
		const phase =
			record.phase === "commentary" || record.phase === "final_answer" ? `:${record.phase}` : "";
