...
import { writeDebugArtifact } from "./debug.js";
...
import { resolveNativeCompactionEnvironment } from "./runtime.js";
import { type ResponsesInputItem, serializeMessagesToCompactRequest } from "./serializer.js";
import {
...
export type CompactionController = {
	getSettings(): ExtensionSettings;
	setSettings(settings: ExtensionSettings): void;
	/** Apply native replay rewrite only. Returns undefined when no rewrite applies. */
...
export function registerCompaction(
	pi: ExtensionAPI,
	initial: PackageSettings["openaiNativeCompaction"],
): CompactionController {
	let settings: ExtensionSettings = { ...initial };
	...
	async function handleSessionBeforeCompact(
		...
		if (latestNativeCompaction.ok) {
			...
			request = serializeMessagesToCompactRequest({
				model: runtime.currentModel,
				messages: buildSessionContext(piContext.sessionManager.getBranch()).messages,
				instructions,
			});
		} else {
			writeDebugArtifact(
				"compaction-event",
				{
					...
					latestCompactionIdentity: getCompactionIdentityDebugInfo(
						latestNativeCompaction.latestCompaction,
					),
				},
				settings,
				piContext,
	...
	async function rewriteProviderRequest(
		event: BeforeProviderRequestEvent,
		ctx: ExtensionContext,
	): Promise<unknown | undefined> {
		if (!settings.enabled) return undefined;
		...
		if (!runtime.payload || !latestNativeCompactionEntry.details) return undefined;
