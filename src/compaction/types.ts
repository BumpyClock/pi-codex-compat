...
export type CreateNativeCompactionDetailsInput = {
	provider: string;
	api: string;
	model: string;
	baseUrl: string;
...
export function isNativeCompactionDetails(value: unknown): value is NativeCompactionDetails {
	...
	if (value.strategy !== NATIVE_COMPACTION_STRATEGY) return false;
	if (!isNativeCompactionIdentity(value)) return false;
	const record = value as Record<string, unknown>;
	return (
		Array.isArray(record.compactedWindow) &&
		record.compactedWindow.every(isCompactedWindowItem) &&
		isNonEmptyString(record.createdAt) &&
		(record.compactResponseId === undefined || isNonEmptyString(record.compactResponseId)) &&
		(record.requestMeta === undefined || isNativeCompactionRequestMeta(record.requestMeta))
	);
}
