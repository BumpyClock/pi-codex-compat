...
const SENSITIVE_KEY_RE =
	/(authorization|api[-_]?key|token|secret|password|cookie|set-cookie|signature|credential|oauth|auth)/i;
const BEARER_RE = /\bBearer\s+[A-Za-z0-9._\-+/=]+/gi;
const OPENAI_KEY_RE = /\bsk-[A-Za-z0-9\-_]+\b/g;
const HEADER_TOKEN_RE = /\b(x-api-key|api-key|authorization)\b\s*[:=]\s*[^\s,;]+/gi;
/** Broad JWT-like scrubbing for arbitrary strings (header.payload.signature). */
...
function toSessionInfo(context: ArtifactContext): ArtifactSessionInfo {
	const maybeExtensionContext = context as Partial<ArtifactSessionInfo> &
	...
	return {
		cwd: context.cwd,
		sessionId: maybeExtensionContext.sessionId,
		sessionFile: maybeExtensionContext.sessionFile,
		sessionDir: maybeExtensionContext.sessionDir,
	};
}
...
function redactInlineSecrets(value: string, placeholder: string): string {
	return value
		...
		.replace(JWT_LIKE_RE, placeholder)
...
export function resolveArtifactPaths(
	...
	const sessionDir = path.join(
		rootDir,
		"sessions",
		sanitizePathSegment(sessionInfo.sessionId, "no-session"),
	);
