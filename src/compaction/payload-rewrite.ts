...
function buildNativeReplaySegmentsInternal<TApi extends Api>(args: {
	...
	if (!details) {
		return {
			ok: false as const,
			reason: "invalid-compacted-window" as const,
			parity: undefined,
		};
	}
	...
	const compactionSummaryCount = serializeMessagesToResponsesInput(args.model, [
		compactionSummaryMessage,
	]).length;
	const preCompactionKeptCount = serializeMessagesToResponsesInput(
		args.model,
		preCompactionKeptMessages,
	).length;
	...
	const actualPostCompactionTail = cloneResponsesInputSlice(
		args.payload.input.slice(tailStartIndex, tailEndIndex),
	);
